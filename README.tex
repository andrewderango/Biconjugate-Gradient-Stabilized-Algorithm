...
\codebox{CFLAGS = -Wall -Wextra -g -lm -O2 \$(shell pkg-config --cflags libpng)}: \codebox{CFLAGS} lists the flags that will be used by the compiler defined above. Each dash represents a precursor for another flag. \codebox{-Wall} enables the compiler to display warning messages upon compilation, such as declared but unused variables within the program. \codebox{-Wextra} provides more potential warnings. \codebox{-g} enables the compiler to provide debugging information, and \codebox{-lm} helps connect the \codebox{math.h} header used for other files. \codebox{-O2} enables possible optimizations within the program to further reduce the computation time. The elements within the brackets configure the \codebox{libpng} package, which is needed for the sparse patterns.
