...
\codebox{CFLAGS= -Wall -Wextra -g -O2 \$(shell pkg-config --cflags libpng)}: CFLAGS lists the flags that will be used by the compiler defined above. Each dash represents a precursor for another flag.  -Wall enables the compiler to display warning messages upon compilation, such as declared but unused variables within the program. -Wextra provides more potential warnings. -g enables the compiler to provide debugging information, and -O2 enables possible optimizations within the program to further reduce the computation time. The elements within the brackets configure the \codebox{libpng} package, which is needed for the sparse patterns.
...
\codebox{bicgstab}, \codebox{main.o}, and \codebox{functions.o}: These compile \codebox{main.c} and \codebox{functions.c} into object files which are dependent on \codebox{functions.h}
